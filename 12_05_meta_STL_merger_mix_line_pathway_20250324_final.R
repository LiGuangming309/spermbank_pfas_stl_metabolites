...
pfas_meta_STL <- read_excel("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/metabolomics/pfas_meta_stl_20241216/meta_stl_result/tidy_result_liner/result_all_meta_STL_zscore_liner_q.xlsx")
names(pfas_meta_STL)
...
###过滤出PFAS-META-STL<0.2的Q值
pfas_meta_STL_1  %>% 
  left_join(df_unique_3,by="metabolite") -> pfas_meta_STL_p_kegg_volcano


font_add("A","C:/Users/zhimo/AppData/Local/Microsoft/Windows/Fonts/Arial.ttf")   ####字体存放位置

volcano_plot_all <-
  pfas_meta_STL_p_kegg_volcano %>% 
  ...
  labs(x = "Metabolism and sperm telomere length correlation", y = "-log10(q-values)")+
  theme_bw() +
...
pfas_meta_STL_p_kegg$metabolite

common_columns <- intersect(colnames(df_rawdata), pfas_meta_STL_p_kegg$metabolite)
print(common_columns)


df_rawdata %>% 
  dplyr::select(number:STL,common_columns) -> df_rawdata_q
...
df_rawdata_q %>% 
  select(1,M485:M623) -> df_meta_final
...
cairo_pdf("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/metabolomics/pfas_meta_stl_20241216/pathway/meta_STL_merger_heatmap_line.pdf",width = 12, height = 10, pointsize = 20,family = "Arial")
opar <- par(no.readonly = TRUE)
par(pin = c(9,16), mar = c(7,7.5,1,1.5))  ##,mgp=c(3,2,0)
...
# pathway_class <-    ####这个函数突然失效了，无法进行计算
...
enrich_scatter_plot(object = result_case_control,
  ...
  geom_text_repel(aes(label = result_case_control@result$pathway_name), size =8, box.padding = 0.3, point.padding = 0.8) +  # 使用 ggrepel 避免标签重叠
  theme_bw() +
...
cairo_pdf("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/metabolomics/pfas_meta_stl_20241216/pathway/pfas_meta_STL_20241218_fdr_network_liner.pdf",width = 11, height = 9,family = "Arial")
opar <- par(no.readonly = TRUE)
par(pin = c(9,16), mar = c(7,7.5,1,1.5))  ##,mgp=c(3,2,0)
...
cairo_pdf("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/metabolomics/pfas_meta_stl_20241216/pathway/pfas_meta_20241218_path_merger_line.pdf",width = 16, height = 12, pointsize = 20,family = "Arial")
opar <- par(no.readonly = TRUE)
par(pin = c(9,16), mar = c(7,7.5,1,1.5))  ##,mgp=c(3,2,0)
