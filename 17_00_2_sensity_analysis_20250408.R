...
###=========================剔除中的异常值（中位数±3IQR ）================
###
quantile(df_background_DNA_LT_sperm_PFOA_tidy$PFOS)
IQR(df_background_DNA_LT_sperm_PFOA_tidy$PFOS)

median(df_background_DNA_LT_sperm_PFOA_tidy$PFOS)+3*IQR(df_background_DNA_LT_sperm_PFOA_tidy$PFOS)##1.18983
median(df_background_DNA_LT_sperm_PFOA_tidy$PFOS)-3*IQR(df_background_DNA_LT_sperm_PFOA_tidy$PFOS)  ##-0.7826699
...
df_background_DNA_LT_sperm_PFOA_tidy %>% 
  dplyr::mutate(
    PFOS_out=case_when(
      PFOS< median(df_background_DNA_LT_sperm_PFOA_tidy$PFOS)-3*IQR(df_background_DNA_LT_sperm_PFOA_tidy$PFOS)  | PFOS>median(df_background_DNA_LT_sperm_PFOA_tidy$PFOS)+3*IQR(df_background_DNA_LT_sperm_PFOA_tidy$PFOS)~1,
      .default = 0
    ),
    PFOA_out=case_when(
      PFOA< median(df_background_DNA_LT_sperm_PFOA_tidy$PFOA)-3*IQR(df_background_DNA_LT_sperm_PFOA_tidy$PFOA)  | PFOA >median(df_background_DNA_LT_sperm_PFOA_tidy$PFOA)+3*IQR(df_background_DNA_LT_sperm_PFOA_tidy$PFOA)~1,
      .default = 0
    ),
    PFDA_out=case_when(
      PFDA< median(df_background_DNA_LT_sperm_PFOA_tidy$PFDA)-3*IQR(df_background_DNA_LT_sperm_PFOA_tidy$PFDA)  | PFDA >median(df_background_DNA_LT_sperm_PFOA_tidy$PFDA)+3*IQR(df_background_DNA_LT_sperm_PFOA_tidy$PFDA)~1,
      .default = 0
    ),
    L_PFHxS_out=case_when(
      L_PFHxS< (median(df_background_DNA_LT_sperm_PFOA_tidy$L_PFHxS)-3*IQR(df_background_DNA_LT_sperm_PFOA_tidy$L_PFHxS))  | L_PFHxS >median(df_background_DNA_LT_sperm_PFOA_tidy$L_PFHxS)+3*IQR(df_background_DNA_LT_sperm_PFOA_tidy$L_PFHxS)~1,
      .default = 0
    ),
    PF3ONS_9CL_out=case_when(
      PF3ONS_9CL< median(df_background_DNA_LT_sperm_PFOA_tidy$PF3ONS_9CL)-3*IQR(df_background_DNA_LT_sperm_PFOA_tidy$PF3ONS_9CL)  | PF3ONS_9CL >median(df_background_DNA_LT_sperm_PFOA_tidy$PF3ONS_9CL)+3*IQR(df_background_DNA_LT_sperm_PFOA_tidy$PF3ONS_9CL)~1,
      .default = 0
    )
  )  -> df_background_DNA_LT_sperm_PFOA_tidy_out


df_background_DNA_LT_sperm_PFOA_tidy_out %>% 
  dplyr::count(PFOS_out)

df_background_DNA_LT_sperm_PFOA_tidy_out %>% 
  dplyr::count(PFOA_out)


df_background_DNA_LT_sperm_PFOA_tidy_out %>% 
  dplyr::count(PFDA_out)

df_background_DNA_LT_sperm_PFOA_tidy_out %>% 
  dplyr::count(L_PFHxS_out)


df_background_DNA_LT_sperm_PFOA_tidy_out %>% 
  dplyr::count(PF3ONS_9CL_out)



str(df_background_DNA_LT_sperm_PFOA_tidy_out)
...
df_background_DNA_LT_sperm_PFOA_tidy_out %>% 
  dplyr::mutate(
    all_out=case_when(
      PFOS_out==1 |PFOA_out==1|PFDA_out==1|L_PFHxS_out==1|PF3ONS_9CL_out==1~1,
      .default = 0
    )) ->df_background_DNA_LT_sperm_PFOA_tidy_out_2
...
df_background_DNA_LT_sperm_PFOA_tidy_out_2 %>% 
  dplyr::filter(all_out==0) -> df_background_DNA_LT_sperm_PFOA_tidy_out_3###先标记离群值再删除，剩下707个样本

str(df_background_DNA_LT_sperm_PFOA_tidy_out_3)


df_background_DNA_LT_sperm_PFOA_tidy_out_3 %>% 
  dplyr::select(-c(PFOS_out:all_out)) -> df_background_DNA_LT_sperm_PFOA_tidy_out_final  ###得到最终的数据


str(df_background_DNA_LT_sperm_PFOA_tidy_out_final)
...
####对PFAS取对数
lapply(df_background_DNA_LT_sperm_PFOA_tidy_out_final[,c(1:14)],log) ->df_background_DNA_LT_sperm_PFOA_tidy_out_final_lg 

str(df_background_DNA_LT_sperm_PFOA_tidy_out_final_lg)

### PFAS和DNA均取对数了
df_background_DNA_LT_sperm_PFOA_tidy_out_final %>% 
  dplyr::select(-c(1:14,25,26)) %>% 
  cbind(df_background_DNA_LT_sperm_PFOA_tidy_out_final_lg) ->df_background_DNA_LT_sperm_PFOA_tidy_final_lg 
...
cairo_pdf( "result/20240407_PFOA_sperm/Multi_linear_regression_20240407/sensity/sensity_1_exclude_median/sperm_TL.pdf",width = 16, height = 9, pointsize = 20,family = "Arial")
...
# library(ggpubr)
# 
# # annotate("text",x=0.5,y=-0.03,size = 10,colour = "black",fontface="bold",face = "bold",label="βs(95%CI):-0.02(-0.09,0.05)")
# # 
# p2+
# annotate("text",x=0.35,y=0.05,size = 6,colour = "black",fontface="bold",face = "bold",label="PFAS mixture\nPercent change (95%CI):5.65(2.53, 8.77)\n italic(P)<0.001")
# 
# ?annotate
# # annotate_figure(p2,top = text_grob("PFAS mixture\nPercent change (95%CI):5.65(2.53, 8.77)\n P<0.001",size = 20,family = "A",face="bold"))
# annotate_picture("text",x=0.5,y=-0.03,size = 10,colour = "black",fontface="bold",face = "bold",label="βs(95%CI):-0.02(-0.09,0.05)")
...
cairo_pdf( "result/20240407_PFOA_sperm/Multi_linear_regression_20240407/sensity/sensity_1_exclude_median/sperm_mtDNA.pdf",width = 16, height = 9, pointsize = 20,family = "Arial")
...
#   dplyr::mutate(
#     PFOA_out=case_when(
#       PFOA< median(df_background_DNA_LT_blood_PFOA_tidy$PFOA)-3*IQR(df_background_DNA_LT_blood_PFOA_tidy$PFOA)  | PFOA >median(df_background_DNA_LT_blood_PFOA_tidy$PFOA)+3*IQR(df_background_DNA_LT_blood_PFOA_tidy$PFOA)~1,
#       .default = 0
#     ),
#     PFOS_out=case_when(
#       PFOS< median(df_background_DNA_LT_blood_PFOA_tidy$PFOS)-3*IQR(df_background_DNA_LT_blood_PFOA_tidy$PFOS)  | PFOS>median(df_background_DNA_LT_blood_PFOA_tidy$PFOS)+3*IQR(df_background_DNA_LT_blood_PFOA_tidy$PFOS)~1,
#       .default = 0
#     ),
#     PFNA_out=case_when(
#       PFNA< median(df_background_DNA_LT_blood_PFOA_tidy$PFNA)-3*IQR(df_background_DNA_LT_blood_PFOA_tidy$PFNA)  | PFNA >median(df_background_DNA_LT_blood_PFOA_tidy$PFNA)+3*IQR(df_background_DNA_LT_blood_PFOA_tidy$PFNA)~1,
#       .default = 0
#     ),
#     PFDA_out=case_when(
#       PFDA< median(df_background_DNA_LT_blood_PFOA_tidy$PFDA)-3*IQR(df_background_DNA_LT_blood_PFOA_tidy$PFDA)  | PFDA >median(df_background_DNA_LT_blood_PFOA_tidy$PFDA)+3*IQR(df_background_DNA_LT_blood_PFOA_tidy$PFDA)~1,
#       .default = 0
#     ),
#     PFUdA_out=case_when(
#       PFUdA< median(df_background_DNA_LT_blood_PFOA_tidy$PFUdA)-3*IQR(df_background_DNA_LT_blood_PFOA_tidy$PFUdA)  | PFUdA >median(df_background_DNA_LT_blood_PFOA_tidy$PFUdA)+3*IQR(df_background_DNA_LT_blood_PFOA_tidy$PFUdA)~1,
#       .default = 0
#     ),
#     L_PFHxS_out=case_when(
#       L_PFHxS< (median(df_background_DNA_LT_blood_PFOA_tidy$L_PFHxS)-3*IQR(df_background_DNA_LT_blood_PFOA_tidy$L_PFHxS))  | L_PFHxS >median(df_background_DNA_LT_blood_PFOA_tidy$L_PFHxS)+3*IQR(df_background_DNA_LT_blood_PFOA_tidy$L_PFHxS)~1,
#       .default = 0
#     ),
#     PF3OUdS_11CL_out=case_when(
#       PF3OUdS_11CL< median(df_background_DNA_LT_blood_PFOA_tidy$PF3OUdS_11CL)-3*IQR(df_background_DNA_LT_blood_PFOA_tidy$PF3OUdS_11CL)  | PF3OUdS_11CL >median(df_background_DNA_LT_blood_PFOA_tidy$PF3OUdS_11CL)+3*IQR(df_background_DNA_LT_blood_PFOA_tidy$PF3OUdS_11CL)~1,
#       .default = 0
#     ),
#     PF3ONS_9CL_out=case_when(
#       PF3ONS_9CL< median(df_background_DNA_LT_blood_PFOA_tidy$PF3ONS_9CL)-3*IQR(df_background_DNA_LT_blood_PFOA_tidy$PF3ONS_9CL)  | PF3ONS_9CL >median(df_background_DNA_LT_blood_PFOA_tidy$PF3ONS_9CL)+3*IQR(df_background_DNA_LT_blood_PFOA_tidy$PF3ONS_9CL)~1,
#       .default = 0
#     ),
#     PFDoA_out=case_when(
#       PFDoA< median(df_background_DNA_LT_blood_PFOA_tidy$PFDoA)-3*IQR(df_background_DNA_LT_blood_PFOA_tidy$PFDoA)  | PFDoA >median(df_background_DNA_LT_blood_PFOA_tidy$PFDoA)+3*IQR(df_background_DNA_LT_blood_PFOA_tidy$PFDoA)~1,
#       .default = 0
#     ),
#     L_PFHpS_out=case_when(
#       L_PFHpS< median(df_background_DNA_LT_blood_PFOA_tidy$L_PFHpS)-3*IQR(df_background_DNA_LT_blood_PFOA_tidy$L_PFHpS)  | L_PFHpS >median(df_background_DNA_LT_blood_PFOA_tidy$L_PFHpS)+3*IQR(df_background_DNA_LT_blood_PFOA_tidy$L_PFHpS)~1,
#       .default = 0
#     ),
#     PFTrDA_out=case_when(
#       PFTrDA< median(df_background_DNA_LT_blood_PFOA_tidy$PFTrDA)-3*IQR(df_background_DNA_LT_blood_PFOA_tidy$PFTrDA)  | PFTrDA >median(df_background_DNA_LT_blood_PFOA_tidy$PFTrDA)+3*IQR(df_background_DNA_LT_blood_PFOA_tidy$PFTrDA)~1,
#       .default = 0
#     ),
#     L_PFBS_out=case_when(
#       L_PFBS< median(df_background_DNA_LT_blood_PFOA_tidy$L_PFBS)-3*IQR(df_background_DNA_LT_blood_PFOA_tidy$L_PFBS)  | L_PFBS >median(df_background_DNA_LT_blood_PFOA_tidy$L_PFBS)+3*IQR(df_background_DNA_LT_blood_PFOA_tidy$L_PFBS)~1,
#       .default = 0
#     ),
#     PFBA_out=case_when(
#       PFBA< median(df_background_DNA_LT_blood_PFOA_tidy$PFBA)-3*IQR(df_background_DNA_LT_blood_PFOA_tidy$PFBA)  | PFBA >median(df_background_DNA_LT_blood_PFOA_tidy$PFBA)+3*IQR(df_background_DNA_LT_blood_PFOA_tidy$L_PFBS)~1,
#       .default = 0
#     )
#   )  -> df_background_DNA_LT_blood_PFOA_tidy_out
# 
# 
# df_background_DNA_LT_blood_PFOA_tidy_out %>% 
#   dplyr::count(PFOS_out)
...
# 
# 
# str(df_background_DNA_LT_blood_PFOA_tidy_out)
# 
# 
# 
# 
...
#   dplyr::mutate(
#     all_out=case_when(
#       PFOA_out==1|PFOS_out==1 |PFNA_out==1|PFDA_out==1|PFUdA_out==1|L_PFHxS_out==1|PF3OUdS_11CL_out==1|PF3ONS_9CL_out==1|PFDoA_out==1|L_PFHpS_out==1|PFTrDA_out==1|L_PFBS_out==1|PFBA_out==1~1,
#       .default = 0
#     )) -> df_background_DNA_LT_blood_PFOA_tidy_out_2
# 
# 
# df_background_DNA_LT_blood_PFOA_tidy_out_2 %>% 
#   dplyr::count(all_out)
...
# 
# df_background_DNA_LT_blood_PFOA_tidy_out_2 %>% 
#   dplyr::filter(all_out==0) -> df_background_DNA_LT_blood_PFOA_tidy_out_3 ###先标记离群值再删除，剩下707个样本
# 
# str(df_background_DNA_LT_blood_PFOA_tidy_out_3)
# 
# 
# df_background_DNA_LT_blood_PFOA_tidy_out_3 %>% 
#   dplyr::select(-c(PFOA_out:all_out)) -> df_background_DNA_LT_blood_PFOA_tidy_out_final  ###得到最终的数据
# 
# 
# str(df_background_DNA_LT_blood_PFOA_tidy_out_final)
# 
# 
# 
# 
...
# 
# ####对PFAS取对数
# lapply(df_background_DNA_LT_blood_PFOA_tidy_out_final  ###得到最终的数据
#        [,c(1:18)],log) ->df_background_DNA_LT_blood_PFOA_tidy_out_final_lg  ###得到最终的数据
# 
# 
# str(df_background_DNA_LT_blood_PFOA_tidy_out_final_lg)
# 
# ### PFAS和DNA均取对数了
# df_background_DNA_LT_blood_PFOA_tidy_out_final %>% 
#   dplyr::select(-c(1:18,29,30)) %>% 
#   cbind(df_background_DNA_LT_blood_PFOA_tidy_out_final_lg) ->df_background_DNA_LT_blood_PFOA_tidy_final_lg 
# 
# 
...
# 
# font_add("A","C:/Users/zhimo/AppData/Local/Microsoft/Windows/Fonts/Arial.ttf")
# cairo_pdf( "result/20240407_PFOA_sperm/Multi_linear_regression_20240407/sensity/sensity_1_exclude_median/blood_TL.pdf",width = 16, height = 9, pointsize = 20,family = "Arial")
# 
# plot(qcboot.fit3, pointwiseref = 1,suppressprint = TRUE) -> p
# 
# p + 
...
#   annotate("text",x=0.15,y=0.15,size = 6,colour = "black",fontface="bold",family="Arial",face = "bold",label="Plasma PFAS mixture:Percent change (95%CI)\n 6.29 (1.00, 11.57); P<0.05")
# 
# dev.off() 
# # 
# 
