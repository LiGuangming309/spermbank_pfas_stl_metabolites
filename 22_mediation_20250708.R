...
df_weighted %>%
  slice(rep(1:n(), each = 849)) %>%
  mutate(ID = rep(1:849, times = nrow(df_weighted)))  %>% 
  arrange(ID)-> df_weighted_1
...
df2 %>% 
  select(-c(PFOS:STL)) %>%    ####删除取对数和标化之后的数据，重新进行匹配并进行标化
   left_join(df_background_DNA_LT_sperm_PFOA_tidy[ ,c(1:6,15)],by="number")  %>% 
  select(number,ID,age_class_2:abstinence_class_2,M661:M625,PFOS:PF3ONS_9CL)-> df2_0   ####将原始的PFAS的浓度值赋值上


# colSums(is.na(df2_0)) -> df2_1
...
write.csv(df_long,"result/20240407_PFOA_sperm/Multi_linear_regression_20240407/metabolomics/pfas_meta_stl_20241216/mediation/metabolomics_estiates_20250707.csv")

names(df_long)



####查看是否对应上PFAS浓度和PFAS的系数
df_long %>% 
  filter(compound=="M661") -> df_long_1

write.csv(df_long_1,"result/20240407_PFOA_sperm/Multi_linear_regression_20240407/metabolomics/pfas_meta_stl_20241216/mediation/df_long_1_20250707.csv")

df_weighted_1 %>% 
  filter(metabolite=="M661") -> df_weighted_2


write.csv(df_weighted_2,"result/20240407_PFOA_sperm/Multi_linear_regression_20240407/metabolomics/pfas_meta_stl_20241216/mediation/df_weighted_2_20250707.csv")
...
df_merged <- df_long %>%
  left_join(df_weighted_1, by = c("ID" = "ID","compound" = "metabolite", "variable" = "var.names.x"))


write.csv(df_merged,"result/20240407_PFOA_sperm/Multi_linear_regression_20240407/metabolomics/pfas_meta_stl_20241216/mediation/df_merged_estiates_20250707.csv")
# 查看合并后的结果
head(df_merged)
...
df_merged_final %>%
  dplyr::group_by(ID,compound) %>%       # 根据 sampleID 和 variable 分组
...
df_background_pfas_stl_zscore[,-c(2:22)] %>% 
  left_join(df3_pfas_meta_stl,by="number") -> df_background_pfas_stl_zscore_final
...
df_background_pfas_stl_zscore_final_tidy %>% 
  select(number:PF3ONS_9CL,matches(paste(pfas_meta_stl_all$metabolite, collapse = "|"))) ->df_background_DNA_LT_meta_mean_final_pfas
...
###m变量
# 选择 df7 中的列并获取列名
selected_columns_2 <- df7[,c(38:55)]
...
# 假设 df_background_DNA_LT 是你的数据框
res2 <- lapply(names(df_clean[, c(20:55)]), function(x) {
  # 获取列名
  result <- df_clean%>%
    dplyr::group_by(number) %>%
    # summarise(mean_value = mean(!!sym(x), na.rm = TRUE)) # 使用 !!sym 来动态引用列
...
df_background_DNA_LT_meta_mean %>%
  # mutate(M1 = as.character(M1)) %>%  # 转换为字符类型
  filter(!is.na(M558))  -> df_background_DNA_LT_meta_mean_final  # 最终进行线性回归分析的只有623个变量
...
df_background_DNA_LT_meta_mean_final  %>% 
  select(-ID) -> df_background_DNA_LT_meta_mean_final_1




# 去除重复行
df_background_DNA_LT_meta_mean_final_1_unique <- df_background_DNA_LT_meta_mean_final_1 %>%
  distinct()



str(df_background_DNA_LT_meta_mean_final_1_unique)
...
# 第一阶段回归模型
fita <- lm(M558 ~ M558.PFAS + age_class_2 + BMI_class_2 + education + marriage_class_2 + income + smk_class_2 + 
             drk_class_2 + season + abstinence_class_2, data=df_background_DNA_LT_meta_mean_final_1_unique)

summary(fita)


# 第二阶段回归模型
fitb <- lm(STL ~ M103.PFAS + M103+ age_class_2 + BMI_class_2 + education + marriage_class_2 + income + 
               smk_class_2 + drk_class_2 + season + abstinence_class_2, data=df_background_DNA_LT_meta_mean_final_1_unique)
...
# 第一阶段回归模型
fita <- lm(M378 ~ M378.PFAS + age_class_2 + BMI_class_2 + education + marriage_class_2 + income + smk_class_2 + 
             drk_class_2 + season + abstinence_class_2, data=df_background_DNA_LT_meta_mean_final_1_unique)
...
# 第二阶段回归模型
fitb <- lm(STL ~ M378.PFAS + M378+ age_class_2 + BMI_class_2 + education + marriage_class_2 + income + 
             smk_class_2 + drk_class_2 + season + abstinence_class_2, data=df_background_DNA_LT_meta_mean_final_1_unique)
# 定义新的print.summary.mediate函数
...
med.out <- mediate(med.fit, out.fit, treat = "catholic", mediator = "score", sims = 100)
