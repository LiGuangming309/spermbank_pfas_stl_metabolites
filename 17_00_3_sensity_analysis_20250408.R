...
df_pfas_623 <- read_excel("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/metabolomics/pfas_meta_stl_20241216/data_preparation/result_pfas_rawdata_623.xlsx")  ###此处的pfas和端粒长度均取对数了，并未进行标化或归一化


df_background_DNA_LT_sperm_PFOA_tidy_final_lg %>% 
  filter(number %in% df_pfas_623$number) -> df_background_DNA_LT_sperm_PFOA_tidy_final_lg
...
lapply(seq_along(res1), function(i) {
  write.xlsx(x = res1[[i]] %>% 
               ...
               filter(term=="x") %>%     ###取出 PFOS等变量 
               cbind(names(res1[[1]]) %>%   ###结合上pfos等变量名称
                       as_tibble()), file = paste0("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/sensity/sensity_sperm_pfas_metabolite_stl/result_Sperm_DNA_aj_2024_",names(res1) %>% as_tibble() %>% slice(i), ".xlsx"), row.names = FALSE)
})
...
###导出保存为pdf
cairo_pdf("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/sensity/sensity_sperm_pfas_metabolite_stl/result_Sperm_DNA_aj_20241019_fina.pdf",width =16 , height =6, pointsize = 20,family = "Arial")
...
###导出保存为pdf
cairo_pdf("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/sensity/sensity_sperm_pfas_metabolite_stl/result_Sperm_DNA_aj_20241019.pdf",width = 12, height = 16, pointsize = 20,family = "Arial")
opar <- par(no.readonly = TRUE)
par(pin = c(9,16), mar = c(7,7.5,1,1.5))  ##,mgp=c(3,2,0)
g <- ggplot_gtable(ggplot_build(p))
stripr <- which(grepl('strip-r', g$layout$name))
...
###绘制剂量反应图
tiff("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/sensity/sensity_sperm_pfas_metabolite_stl/result_Sperm_DNA_aj_20241019.tiff",height = 1280,width =960)
opar <- par(no.readonly = TRUE)
par(pin = c(9,16), mar = c(7,7.5,1,1.5))  ##,mgp=c(3,2,0)
g <- ggplot_gtable(ggplot_build(p))
stripr <- which(grepl('strip-r', g$layout$name))
...
df_background_DNA_LT_sperm_PFOA_tidy_less_LOD %>% 
  filter(number %in% df_pfas_623$number)  -> df_background_DNA_LT_sperm_PFOA_tidy_less_LOD
...
write.xlsx(res_all,"result/20240407_PFOA_sperm/Multi_linear_regression_20240407/sensity/sensity_sperm_pfas_metabolite_stl/result_Sperm_DNA_aj_2024_IQR.xlsx")
...
write.xlsx(res_all_2,"result/20240407_PFOA_sperm/Multi_linear_regression_20240407/sensity/sensity_sperm_pfas_metabolite_stl/result_Sperm_DNA_aj_2024_IQR_final.xlsx")
...
###导出保存为pdf
cairo_pdf("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/sensity/sensity_sperm_pfas_metabolite_stl/result_Sperm_DNA_aj_20241019_IQR_fina.pdf",width =16 , height =9, pointsize = 20,family = "Arial")
opar <- par(no.readonly = TRUE)
par(pin = c(9,16), mar = c(7,7.5,1,1.5))  ##,mgp=c(3,2,0)
p3
par(opar)
...
# windowsFonts(A = windowsFont("Calibri")) 
###绘制剂量反应图
cairo_pdf("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/sensity/sensity_sperm_pfas_metabolite_stl/result_Sperm_DNA_aj_20241019_IQR_fina_color.pdf",width =16 , height =9, pointsize = 20,family = "Arial")
opar <- par(no.readonly = TRUE)
par(pin = c(9,16), mar = c(7,7.5,1,1.5))  ##,mgp=c(3,2,0)
###修改分面中每个标题的颜色
g <- ggplot_gtable(ggplot_build(p3))
...
cairo_pdf("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/sensity/sensity_sperm_pfas_metabolite_stl/sperm_DNA.pdf",width = 16, height = 9, pointsize = 20,family = "Arial")
...
cairo_pdf("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/sensity/sensity_sperm_pfas_metabolite_stl/sperm_TL.pdf",width = 16, height = 9, pointsize = 20,family = "Arial")
