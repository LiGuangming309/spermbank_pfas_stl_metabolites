...
lapply(df_background_DNA_LT_blood_PFOA_tidy_final_lg[,c(27:28)], function(y){
  lapply(df_background_DNA_LT_blood_PFOA_tidy_final_lg[,c(11:26)], function(x){
    lm(y~x+age_class_2+BMI_class_2+education+marriage_class_2+income+smk_class_2+drk_class_2+season+abstinence_class_2,data=df_background_DNA_LT_blood_PFOA_tidy_final_lg) -> result
    cox <- summary(result)
...
lapply(seq_along(res1), function(i) {
  write.xlsx(x = res1[[i]] %>% 
               ...
               filter(term=="x") %>%     ###取出 PFOS等变量 
               cbind(names(res1[[1]]) %>%   ###结合上pfos等变量名称
                       as_tibble()), file = paste0("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/adjusted_model/20240525_Blood_PFAS_DNA/result_blood_DNA_aj_2024_",names(res1) %>% as_tibble() %>% slice(i), ".xlsx"), row.names = FALSE)
})
...
res2[[1]] %>% 
  cbind(rep(1,times=16) %>% tibble() %>% dplyr::rename(group=".")) -> res_DNA  ###重复生成一列数据



res2[[2]] %>% 
  cbind(rep(2,times=16) %>% tibble() %>% dplyr::rename(group=".")) -> res_STL  ###重复生成一列数据
...
### 过滤掉LOD太低的数值
res_DNA_STL_meg_1  %>% 
  filter(value %in% c("PFOA","PFOS","PFNA","PFDA","PFUdA","L_PFHxS","diPAP_6_2","PF3OUdS_11CL","PF3ONS_9CL","PFDoA","L_PFHpS")) -> res_DNA_STL_meg_2
...
show_col(pal_aaas("default", alpha = 1)(11))  ####查看NEJM杂志的配色，并进行着色

show_col(pal_lancet(palette = c("lanonc"), alpha = 1)(11))  ####查看NEJM杂志的配色，并进行着色
...
# dev.off()
 p1 <- res_DNA_STL_meg_3 %>% 
  mutate(value_term=case_when(
    ...
    value=="PFUdA" ~"PFUdA",
    value=="L_PFHxS" ~"PFHxS",
    value=="diPAP_6_2" ~"6:2 diPAP",
    value=="PF3ONS_9CL" ~"6:2 Cl-PFESA",  ###修改为文章的变量名称
    value=="PF3OUdS_11CL" ~"8:2 Cl-PFESA", 
    value=="PFDoA" ~"PFDoA", 
    value=="L_PFHpS" ~"PFHpS"
  ))  %>% 
...
###导出保存为pdf
cairo_pdf("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/adjusted_model/20240525_Blood_PFAS_DNA/result_blood_DNA_aj_20241019_fina.pdf",width =16 , height =6, pointsize = 20,family = "Arial")
...
res_DNA_STL_meg_3 %>% 
  ...
  # coord_flip()+
  facet_grid(value_term ~ ., scales = "free_y")+
...
fills <- c("#BC3C29FF","#0072B5FF","#E18727FF","#20854EFF","#7876B1FF","#6F99ADFF","#FFDC91FF","#EE4C97FF","#0099B4FF","#ADB6B6FF","#FDAF91FF")
k <- 1
for (i in stripr) {
...
###导出保存为pdf
cairo_pdf("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/adjusted_model/20240525_Blood_PFAS_DNA/result_blood_DNA_aj_20241019.pdf",width = 12, height = 16, pointsize = 20,family = "Arial")
opar <- par(no.readonly = TRUE)
par(pin = c(9,16), mar = c(7,7.5,1,1.5))  ##,mgp=c(3,2,0)
###修改分面中每个标题的颜色
g <- ggplot_gtable(ggplot_build(p))
...
###将"PFTrDA","L_PFBS"转换为三分类变量
lapply(df_background_DNA_LT_blood_PFOA_tidy_final_lg[,c("PFTrDA","L_PFBS")], function(x){ 
  cut(x,breaks =quantile(x,probs=c(0.0,0.70,0.85,1.00)),include.lowest=T,
      labels = c(1,2,3))%>% 
    as_tibble() 
})  %>%   bind_cols() %>%    ####将所有列数据合并在一个数据框中
  set_names(paste(data.frame(colnames(df_background_DNA_LT_blood_PFOA_tidy_final_lg))[c(22,23),],"_","class",sep = "")) ->df_background_DNA_LT_blood_PFOA_tidy_final_lg_class_2  



###将PFBA转换为三分类变量
cut(df_background_DNA_LT_blood_PFOA_tidy_final_lg$PFBA,breaks =quantile(df_background_DNA_LT_blood_PFOA_tidy_final_lg$PFBA,probs=c(0.0,0.60,0.80,1.00)),include.lowest=T,
    labels = c(1,2,3))%>% 
  bind_cols() %>%    ####将所有列数据合并在一个数据框中
  set_names(paste(data.frame(colnames(df_background_DNA_LT_blood_PFOA_tidy_final_lg))[c(24),],"_","class",sep = "")) ->df_background_DNA_LT_blood_PFOA_tidy_final_lg_class_3  
...
df_background_DNA_LT_blood_PFOA_tidy_final_lg %>% 
  cbind(df_background_DNA_LT_blood_PFOA_tidy_final_lg_class,df_background_DNA_LT_blood_PFOA_tidy_final_lg_class_2,df_background_DNA_LT_blood_PFOA_tidy_final_lg_class_3) -> df_background_DNA_LT_blood_PFOA_tidy_final_LOD_Class
...
df_background_DNA_LT_blood_PFOA_tidy %>% 
  mutate(
    ...
    PFNA_lod=case_when(
      PFNA < 0.045045045045045 ~ 0,
      PFNA>=0.045045045045045~ 1
    ),
    PFDA_lod=case_when(
    ...
    diPAP_6_2_lod=case_when(
      diPAP_6_2< 0.00221141088014153~0,
      diPAP_6_2>=0.00221141088014153~1
    ),
    ...
    PFDoA_lod=case_when(
      PFDoA< 0.00255493101686255~0,
      PFDoA>=0.00255493101686255~1
    ),
    L_PFHpS_lod=case_when(
      L_PFHpS< 0.00917992656058752~0,
      L_PFHpS>=0.00917992656058752~1
    ),
    PFTrDA_lod=case_when(
      PFTrDA< 0.00586969281940912~0,
      PFTrDA>=0.00586969281940912~1
    ),
    L_PFBS_lod=case_when(
      L_PFBS< 0.00494967827091239~0,
      L_PFBS>=0.00494967827091239~1
    ),
    PFBA_lod=case_when(
      PFBA< 0.0319216854649925~0,
      PFBA>=0.0319216854649925~1
    )
  ) -> df_background_DNA_LT_blood_PFOA_tidy_less_LOD

str(df_background_DNA_LT_blood_PFOA_tidy_less_LOD)

summary(df_background_DNA_LT_blood_PFOA_tidy_less_LOD)

####将生成的LOD转换为因变量并连接到原来的数据框

df_background_DNA_LT_blood_PFOA_tidy_final_LOD_Class %>% 
  cbind(
    lapply(df_background_DNA_LT_blood_PFOA_tidy_less_LOD[,c(31:44)], factor)  %>% 
      bind_cols()  %>%    ####将所有列数据合并在一个数据框中
      set_names(paste(data.frame(colnames(df_background_DNA_LT_blood_PFOA_tidy_less_LOD))[c(31:44),],"_","factor",sep = "")) 
  ) ->  df_background_DNA_LT_blood_PFOA_tidy_final_LOD_Class_final_2

summary(df_background_DNA_LT_blood_PFOA_tidy_final_LOD_Class_final_2)

library(dplyr)
library(tidyr)

str(df_background_DNA_LT_blood_PFOA_tidy_final_LOD_Class_final_2)
...
###PFNA
df_background_DNA_LT_blood_PFOA_tidy_final_LOD_Class_final_2 %>% 
  dplyr::group_by(PFNA_class,PFNA_lod_factor) %>% 
  tally() 
1021-9
...
###diPAP_6_2
df_background_DNA_LT_blood_PFOA_tidy_final_LOD_Class_final_2 %>% 
  dplyr::group_by(diPAP_6_2_class,diPAP_6_2_lod_factor) %>% 
  tally() 
1021-132
...
###PFDoA
df_background_DNA_LT_blood_PFOA_tidy_final_LOD_Class_final_2 %>% 
  dplyr::group_by(PFDoA_class,PFDoA_lod_factor) %>% 
  tally() 
1021-120
...
###L_PFHpS
df_background_DNA_LT_blood_PFOA_tidy_final_LOD_Class_final_2 %>% 
  dplyr::group_by(L_PFHpS_class,L_PFHpS_lod_factor) %>% 
  tally() 
...
###PFTrDA
df_background_DNA_LT_blood_PFOA_tidy_final_LOD_Class_final_2 %>% 
  dplyr::group_by(PFTrDA_class,PFTrDA_lod_factor) %>% 
  tally() 
...
###L_PFBS
df_background_DNA_LT_blood_PFOA_tidy_final_LOD_Class_final_2 %>% 
  dplyr::group_by(L_PFBS_class,L_PFBS_lod_factor) %>% 
  tally() 
...
###PFBA
df_background_DNA_LT_blood_PFOA_tidy_final_LOD_Class_final_2 %>% 
  dplyr::group_by(PFBA_class,PFBA_lod_factor) %>% 
  tally() 
...
###血浆浆检出率大于80%的有PFOS+PFOA+PFUdA+L_PFHxS+PF3ONS_9CL
##11,12,14,
...
lapply(df_background_DNA_LT_blood_PFOA_tidy_final_LOD_Class[,c(27:28)], function(y){
  lapply(df_background_DNA_LT_blood_PFOA_tidy_final_LOD_Class[,c(29:42)], function(x){
    lm(y~x+age_class_2+BMI_class_2+education+marriage_class_2+income+smk_class_2+drk_class_2+season+abstinence_class_2,data=df_background_DNA_LT_blood_PFOA_tidy_final_LOD_Class) -> result
    cox <- summary(result)
    cbind(cox$coefficients[,c(1,2,3,4)],confint(result)) -> result2
    β<-round(cox$coefficients[,1],digits = 2)
    CI95<-paste0(β,"(",round(result2[,5],2),',',round(result2[,6],2),")")
...
# ###然后再将生成的list重新保存为独立的excel文件
# lapply(seq_along(res1), function(i) {
#   write.xlsx(x = res1[[i]], file = paste0("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/adjusted_model/20240525_Blood_PFAS_DNA/四分位间距/result_Blood_DNA_4_class_aj_",names(res1) %>% as_tibble() %>% slice(i), ".xlsx"), row.names = FALSE)
# })
...
write.xlsx(res_all,"result/20240407_PFOA_sperm/Multi_linear_regression_20240407/adjusted_model/20240525_Blood_PFAS_DNA/result_blood_DNA_aj_2024_IQR.xlsx")
...
write.xlsx(res_all_2,"result/20240407_PFOA_sperm/Multi_linear_regression_20240407/adjusted_model/20240525_Blood_PFAS_DNA/result_blood_DNA_aj_2024_IQR_final.xlsx")
...
res_all_2 %>% 
  filter(dataframe_name  %in%  c("PFOA_class","PFOS_class","PFNA_class","PFDA_class","PFUdA_class","L_PFHxS_class","diPAP_6_2_class","PF3OUdS_11CL_class","PF3ONS_9CL_class","PFDoA_class","L_PFHpS_class")) -> res_all_3
...
res_all_3  %>% 
  dplyr::mutate(value_term=case_when(
    ...
    dataframe_name=="PFNA_class" ~"PFNA",
    dataframe_name=="PFDA_class" ~"PFDA",
    dataframe_name=="PFUdA_class" ~"PFUdA",
    dataframe_name=="L_PFHxS_class" ~"PFHxS",
    dataframe_name=="diPAP_6_2_class" ~"6:2 diPAP",
    dataframe_name=="PF3ONS_9CL_class" ~"6:2 Cl-PFESA",  ###修改为文章的变量名称
    dataframe_name=="PF3OUdS_11CL_class" ~"8:2 Cl-PFESA", 
    dataframe_name=="PFDoA_class" ~"PFDoA", 
    dataframe_name=="L_PFHpS_class" ~"PFHpS"),
  term_Q=case_when(
    term=="x1"~ "Q1",
    term=="x2"~ "Q2",
    term=="x3"~ "Q3",
...
###导出保存为pdf
cairo_pdf("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/adjusted_model/20240525_Blood_PFAS_DNA/result_blood_DNA_aj_20241019_IQR_fina.pdf",width =16 , height =9, pointsize = 20,family = "Arial")
opar <- par(no.readonly = TRUE)
par(pin = c(9,16), mar = c(7,7.5,1,1.5))  ##,mgp=c(3,2,0)
p3
par(opar)
...
# windowsFonts(A = windowsFont("Calibri")) 
###绘制剂量反应图
cairo_pdf("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/adjusted_model/20240525_Blood_PFAS_DNA/result_blood_DNA_aj_20241019_IQR_fina_color.pdf",width =16 , height =9, pointsize = 20,family = "Arial")
opar <- par(no.readonly = TRUE)
par(pin = c(9,16), mar = c(7,7.5,1,1.5))  ##,mgp=c(3,2,0)
###修改分面中每个标题的颜色
g <- ggplot_gtable(ggplot_build(p3))
