...
res2 <- lapply(names(df2_meta_stl[, c(11:295)]), function(x) {    ###这里对PFAS和代谢数据均求均值了，但是PFAS数据并未进行标化
  # 获取列名
  result <- df2_meta_stl %>%
...
df_background_DNA_LT_meta_mean %>% 
  ...
  filter(!is.na(M661))  -> df_background_DNA_LT_meta_mean_final  # 过滤掉 NA
...
write.xlsx(df_background_DNA_LT_meta_mean_final,"result/20240407_PFOA_sperm/Multi_linear_regression_20240407/metabolomics/pfas_meta_stl_20241216/data_preparation/result_pfas_rawdata_623.xlsx")  ###此处的pfas和端粒长度均取对数了，并未进行标化或归一化
