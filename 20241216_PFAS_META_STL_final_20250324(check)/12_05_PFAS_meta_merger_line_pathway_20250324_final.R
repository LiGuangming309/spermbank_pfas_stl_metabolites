...
pfas_meta_STL <- read_excel("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/metabolomics/pfas_meta_stl_20241216/pfas_meta_result/tidy_result/results_df_all_p_value_mix_liner.xlsx")
names(pfas_meta_STL)

pfas_meta_STL$meta

pfas_meta_STL%>% 
  dplyr::rename(metabolite="metabolite") -> pfas_meta_STL_1  
...
volcano_plot_all <-
  ...
  labs(x = "Correlation", y = "-log10(q-values)")+
  theme_bw() +
...
# 
font_add("A","C:/Users/zhimo/AppData/Local/Microsoft/Windows/Fonts/Arial.ttf")   ####字体存放位置
cairo_pdf("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/metabolomics/pfas_meta_stl_20241216/pathway/volcano_pfas_meta_STL_merger_line.pdf",width = 12, height = 10, pointsize = 20,family = "Arial")
...
df_rawdata <- read_excel("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/metabolomics/pfas_meta_stl_20241216/data_preparation/df_background_pfas_stl_zscore_final_tidy.xlsx")
...
df_rawdata_q %>% 
  select(1,M661:M777) -> df_meta_final
...
cairo_pdf("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/metabolomics/pfas_meta_stl_20241216/pathway/meta_STL_1203_merger_heatmap_line.pdf",width = 12, height = 10, pointsize = 20,family = "Arial")
opar <- par(no.readonly = TRUE)
par(pin = c(9,16), mar = c(7,7.5,1,1.5))  ##,mgp=c(3,2,0)
...
# metpath::pathway_class()

library(metid)
# pathway_class <- metpath::pathway_class(kegg_human_pathway)
# 
# 
...
#   dplyr::mutate("x_axis" = result_case_control@result[, x_axis]) %>%
#   dplyr::mutate("y_axis" = result_case_control@result[, y_axis]) %>%
#   dplyr::mutate("point_size" = result_case_control@result[, point_size]) %>%
#   # dplyr::rename("x_axis" = x_axis) %>%
#   # dplyr::rename("y_axis" = y_axis) %>%
#   # dplyr::rename("point_size" = point_size) %>%
#   dplyr::select(pathway_name, x_axis, y_axis, point_size) %>%
...
enrich_scatter_plot(object = result_case_control,
  ...
  geom_text_repel(aes(label = result_case_control@result$pathway_name), size =8, box.padding = 0.6, point.padding = 0.8) +  # 使用 ggrepel 避免标签重叠
  theme_bw() +
  theme(legend.text=element_text(size =20,colour = "black",family = "Arial",face = "bold"),
        ...
        # axis.line = element_line(linewidth = 0.9),
        # axis.ticks = element_line(linewidth = 0.9),
        axis.title.y = element_text(size = 20,colour = "black",family = "Arial",face = "bold"),
...
result_case_control@result$pathway_name
...
cairo_pdf("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/metabolomics/pfas_meta_stl_20241216/pathway/pfas_meta_STL_20241218_scater_merger_liner.pdf",width = 8, height = 8, pointsize = 20,family = "Arial")
opar <- par(no.readonly = TRUE)
par(pin = c(9,16), mar = c(7,7.5,1,1.5))  ##,mgp=c(3,2,0)
...
enrich_scatter_plot(object = result_case_control,
                    ...
                    label = FALSE,
                    label_size = 8)+
  geom_hline(yintercept = -log(0.2,10), linetype = "dashed",color = "red", size = 1) +
  geom_text_repel(aes(label = result_case_control@result$pathway_name), size =8, box.padding = 0.6, point.padding = 0.8) +  # 使用 ggrepel 避免标签重叠
  theme_bw() +
  theme(legend.text=element_text(size =20,colour = "black",family = "Arial",face = "bold"),
        # legend.position = c(0.25,0.15),
...
cairo_pdf("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/metabolomics/pfas_meta_stl_20241216/pathway/pfas_meta_STL_20241218_fdr_bubble_merger_liner.pdf",width = 12, height = 9, pointsize = 20,family = "Arial")
opar <- par(no.readonly = TRUE)
par(pin = c(9,16), mar = c(7,7.5,1,1.5))  ##,mgp=c(3,2,0)
...
result_case_control@result %>% 
  as_tibble() %>% 
  write.csv("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/metabolomics/pfas_meta_stl_20241216/pathway/pfas_meta_20241218_kegg_meta_fdr_merger_liner.csv")
...
meta_STL_mix_kegg_meta_final_3 %>% 
  dplyr::rename("estimate.x"="estimate",
                "estimate_lcl.x"="estimate_lcl",
                "estimate_ucl.x"="estimate_ucl") -> meta_STL_mix_kegg_meta_final_4
...
cairo_pdf("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/metabolomics/pfas_meta_stl_20241216/pathway/pfas_meta_20241218_path_merger_line.pdf",width = 16, height = 12,family = "Arial")
opar <- par(no.readonly = TRUE)
par(pin = c(9,16), mar = c(7,7.5,1,1.5))  ##,mgp=c(3,2,0)
...
#   mutate(group=rep("STL",11))-> meta_STL_mix_kegg_meta_final_4_pfas_1
...
#   mutate(group=rep("PFAS",11))-> meta_STL_mix_kegg_meta_final_4_pfas_2
# 
...
# 
# 
# cairo_pdf("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/metabolomics/pfas_meta_stl_20241216/pathway/pfas_meta_STL_20241218_path_merger_line_merger.pdf",width = 16, height = 12, pointsize = 20,family = "Arial")
# opar <- par(no.readonly = TRUE)
# par(pin = c(9,16), mar = c(7,7.5,1,1.5))  ##,mgp=c(3,2,0)
# 
# p4 # 使用大写罗马表示
