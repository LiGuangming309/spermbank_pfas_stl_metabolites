...
###删除前2行数据
df_sperm_PFOA %>% 
  dplyr::slice(-c(1:2)) ->df_sperm_PFOA_tidy 
...
df_background_DNA_LT_sperm_PFOA_noNA %>% 
  dplyr::select(PFOS:PF3OUdS_11CL,mtDNAcn,STL,everything()) -> df_background_DNA_LT_sperm_PFOA_tidy
...
df_unique_3$metabolite     ###最终得到271个代谢物

357-271

###筛选出最终的代谢物
df_meta_1 %>% 
  dplyr::select(number,sampledate,df_unique_3$metabolite)  ->  df_meta_2  ####最终得到378个代谢物质
...
df_background_DNA_LT_sperm_PFOA_tidy_final_lg  %>% 
  left_join(df_background_repeatsemen_final2,by="number") -> df_background_DNA_LT_sperm_PFOA_meta_tidy
...
table(df_background_DNA_LT_sperm_PFOA_meta_tidy_noNA$interval) 
...
unique(df_background_DNA_LT_sperm_PFOA_meta_tidy_noNA$number) %>% 
  as_tibble()

write.xlsx(df_background_DNA_LT_sperm_PFOA_meta_tidy_noNA,"article/Third_artical_PFAS_DNA/manuscript/revised_wangyixin/df_background_pfas_STL_rawdata.xlsx") 
