...
# Functions
BHRMA.g <- function(X=NULL, Y=NULL, U=NULL, LOD=NULL,id=NULL, profiles=NULL) {
  # JAGS model
  ridge.BDL.model <- 
    "model {
  for(i in 1:N) {
  ...
  univariate.results <- t(sapply(1:P, FUN=function(p) {  # using index p facilitate write
    x <- as.matrix(X[,p])
    reg <- lme4::lmer(Y~x+(1|id))    # perform logistic regression 
...
############################读入数据#################################
df_background_DNA_LT_sperm_PFOA_meta_tidy_noNA <- read_excel("/dssg/home/acct-yxwang0203/yxwang0203/work_data/pfas_STL_meta/pfas_20250324/df_background_pfas_meta_mix_rawdata.xlsx")
...
Y = df_background_DNA_LT_sperm_PFOA_meta_tidy_noNA %>% 
  ...
  `+`(1) %>%
  log2()%>%
  scale(center = T, scale = T)   ####混合线性回归分析代谢化合物未进行对数转化和归一化，所以只是将这里的数据格式进行转化
# Y
...
g_coefs_PFAS_meta_mix_liner <- allgather(coefs)

if(comm.rank() == 0){
  save(g_coefs_PFAS_meta_mix_liner, file = 'coefs_PFAS_mix_liner.Rdata')
}
