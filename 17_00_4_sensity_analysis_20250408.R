...
pfas_meta <- read_excel("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/metabolomics/pfas_meta_stl_20241216/pfas_meta_result/tidy_result/results_df_all_p_value.xlsx")
...
write.xlsx(pfas_meta_STL_p_kegg,"result/20240407_PFOA_sperm/Multi_linear_regression_20240407/sensity/sensity_4_mean_line_pfas/pfas_meta_stl_meeting_q.xlsx")
...
####利用全部的代谢化合物绘制火山图
pfas_meta  %>% 
  left_join(meta_STL,by=c("metabolite"="meta")) -> pfas_meta_stl_2

colSums(is.na(pfas_meta_stl_2))

pfas_meta_stl_2 %>% 
  filter(!Estimate=="NA") -> pfas_meta_stl_all_2 
...
###（2）取出PFAS数据
pfas_meta_STL_p_kegg_2 %>% 
  select(metabolite,estimate.x,estimate_lcl,estimate_ucl,name,p_value,q_value.x) %>% 
  mutate(group=rep("PFAS",271))-> pfas_meta_STL_p_kegg_2_pfas
...
colnames(pfas_meta_STL_p_kegg_2_pfas) <- colnames(pfas_meta_STL_p_kegg_2_stl)    ###将pfas_STL_mix_kegg_meta_final_2的名称调整为colnamespfas_STL_mix_kegg_meta_final_1

pfas_meta_STL_p_kegg_2_pfas

###（3）将PFAS与STL进行列合并
rbind(pfas_meta_STL_p_kegg_2_pfas,pfas_meta_STL_p_kegg_2_stl) -> pfas_meta_STL_p_kegg_2_pfas_stl ##将调整后的列名称数据进行合并

names(pfas_meta_STL_p_kegg_2_pfas_stl)
...
pfas_meta_STL_p_kegg_2_pfas_stl %>% 
  group_by(group) %>% 
  mutate(marker=case_when(
    q_value.y < 0.2 & Estimate  > 0 ~ "Up",   ###p小于0.05，且相关系数大于0代表上调
    q_value.y< 0.2 & Estimate < 0 ~ "Down",    ###p小于0.05，且相关系数小于0代表下调
    TRUE ~ "No"
  )) %>%
  ggplot(aes(x =Estimate, y = -log10(q_value.y),color=marker,shape=group)) + #x、y轴取值限制，颜色根据"Sig"
  # geom_point(alpha=0.65, size=2) +  #点的透明度、大小
  geom_point(aes(size = -log(q_value.y, 10),color = marker),alpha = 0.7) +
...
# 
font_add("A","C:/Users/zhimo/AppData/Local/Microsoft/Windows/Fonts/Arial.ttf")   ####字体存放位置
cairo_pdf("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/sensity/sensity_4_mean_line_pfas/volcano_pfas_meta_STL_merger_line.pdf",width = 7, height = 6, pointsize = 20,family = "Arial")
...
df_rawdata_q %>% 
  select(1,M332:M352) -> df_meta_final
...
cairo_pdf("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/sensity/sensity_4_mean_line_pfas/meta_STL_1203_merger_heatmap_line.pdf",width = 6, height = 5, pointsize = 20,family = "Arial")
opar <- par(no.readonly = TRUE)
par(pin = c(9,16), mar = c(7,7.5,1,1.5))  ##,mgp=c(3,2,0)
...
cairo_pdf("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/sensity/sensity_4_mean_line_pfas/pfas_meta_STL_20241218_bar_merger_line.pdf",width = 12, height = 14, pointsize = 20,family = "Arial")
opar <- par(no.readonly = TRUE)
par(pin = c(9,16), mar = c(7,7.5,1,1.5))  ##,mgp=c(3,2,0)
...
cairo_pdf("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/sensity/sensity_4_mean_line_pfas/pfas_meta_STL_20241218_fdr_bar_merger_line.pdf",width = 12, height = 9, pointsize = 20,family = "Arial")
opar <- par(no.readonly = TRUE)
par(pin = c(9,16), mar = c(7,7.5,1,1.5))  ##,mgp=c(3,2,0)
...
cairo_pdf("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/sensity/sensity_4_mean_line_pfas/pfas_meta_STL_20241218_scater_merger_liner.pdf",width = 12, height = 12, pointsize = 20,family = "Arial")
opar <- par(no.readonly = TRUE)
par(pin = c(9,16), mar = c(7,7.5,1,1.5))  ##,mgp=c(3,2,0)
...
cairo_pdf("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/sensity/sensity_4_mean_line_pfas/pfas_meta_STL_20241218_fdr_scater_merger_liner.pdf",width = 12, height = 12, pointsize = 20,family = "Arial")
opar <- par(no.readonly = TRUE)
par(pin = c(9,16), mar = c(7,7.5,1,1.5))  ##,mgp=c(3,2,0)
...
ggplot(data_kegg_2,aes(x=mapped_percentage,y=pathway_name))+
  ...
  coord_fixed(ratio=5/1)  -> p3_bubble

p3_bubble


cairo_pdf("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/sensity/sensity_4_mean_line_pfas/pfas_meta_STL_20241218_bubble_merger_liner.pdf",width = 12, height = 12, pointsize = 20,family = "Arial")
opar <- par(no.readonly = TRUE)
par(pin = c(9,16), mar = c(7,7.5,1,1.5))  ##,mgp=c(3,2,0)
...
cairo_pdf("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/sensity/sensity_4_mean_line_pfas/pfas_meta_STL_20241218_fdr_bubble_merger_liner.pdf",width = 12, height = 12, pointsize = 20,family = "Arial")
opar <- par(no.readonly = TRUE)
par(pin = c(9,16), mar = c(7,7.5,1,1.5))  ##,mgp=c(3,2,0)
...
result_case_control@result %>% 
  as_tibble() %>% 
  write.csv("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/sensity/sensity_4_mean_line_pfas/pfas_meta_STL_kegg_meta_fdr_merger_liner.csv")
...
cairo_pdf("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/sensity/sensity_4_mean_line_pfas/pfas_meta_STL_20241218_fdr_network_liner.pdf",width = 8, height = 9,family = "Arial")
opar <- par(no.readonly = TRUE)
par(pin = c(9,16), mar = c(7,7.5,1,1.5))  ##,mgp=c(3,2,0)
...
cairo_pdf("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/sensity/sensity_4_mean_line_pfas/pfas_meta_STL_20241218_q_circle_merger_line.pdf",width = 20, height = 20, pointsize = 20,family = "Arial")
opar <- par(no.readonly = TRUE)
par(pin = c(9,16), mar = c(7,7.5,1,1.5))  ##,mgp=c(3,2,0)
p2_circle # 使用大写罗马表示
par(opar)
...
meta_STL_mix_kegg_meta_final_3 %>% 
  dplyr::rename("estimate.z"="Estimate",
                "estimate_lcl.z"="CI2.5",
                "estimate_ucl.z"="CI97.5") -> meta_STL_mix_kegg_meta_final_4
...
meta_STL_mix_kegg_meta_final_4 %>% 
  ...
  ggplot(aes(x = name, y = estimate.z,color=marker)) + 
  geom_point(size = 1.5, position = position_dodge(width = 0.4),shape=15) +           # 绘制β值的点
  geom_errorbar(aes(ymin = estimate_lcl.z, ymax = estimate_ucl.z), 
...
cairo_pdf("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/sensity/sensity_4_mean_line_pfas/pfas_meta_STL_20241218_path_merger_line.pdf",width = 16, height = 12, pointsize = 20,family = "Arial")
opar <- par(no.readonly = TRUE)
par(pin = c(9,16), mar = c(7,7.5,1,1.5))  ##,mgp=c(3,2,0)
...
df_pfas_mix_meta <- read_excel("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/metabolomics/pfas_meta_stl_20241216/pfas_meta_result/tidy_result/results_df_all_p_value.xlsx")
...
names(meta_STL_mix_kegg_meta_final_4_pfas)
###3.将横向数据整理为纵向数据、进行绘图

###（1）取出STL数据 
meta_STL_mix_kegg_meta_final_4_pfas %>% 
  select(metabolite,node_name,name,estimate.z,estimate_lcl.z,estimate_ucl.z,p.value,q_value.x) %>% 
  mutate(group=rep("STL",6))-> meta_STL_mix_kegg_meta_final_4_pfas_1
...
###（2）取出PFAS数据
meta_STL_mix_kegg_meta_final_4_pfas %>% 
  select(metabolite,node_name,name,estimate,estimate_lcl.y,estimate_ucl.y,p.val.y,q_value) %>% 
  mutate(group=rep("PFAS",6))-> meta_STL_mix_kegg_meta_final_4_pfas_2
...
meta_STL_mix_kegg_meta_final_4_pfas_3 %>% 
  ...
  ggplot(aes(x = name, y = estimate.z,color = group)) + 
  geom_point(size = 1.5, position = position_dodge(width = 0.4),shape=15) +           # 绘制β值的点
  geom_errorbar(aes(ymin = estimate_lcl.z, ymax = estimate_ucl.z), 
                width = 0.2, size = 1, position = position_dodge(width = 0.4)) +  # 绘制置信区间
  geom_hline(yintercept = 0, linetype = "dashed", color = "red",size=1) +  # 基线
...
cairo_pdf("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/sensity/sensity_4_mean_line_pfas/pfas_meta_STL_20241218_path_merger_line_merger.pdf",width = 16, height = 12, pointsize = 20,family = "Arial")
opar <- par(no.readonly = TRUE)
par(pin = c(9,16), mar = c(7,7.5,1,1.5))  ##,mgp=c(3,2,0)
...
ggplot() +
  ...
  annotate("text", x = 1.6, y = 2.8, label = "44 \n (41.5%)", size = 5, color = "black",family = "Arial",face = "bold") +
  annotate("text", x = 4.6, y = 2.8, label = "49 \n (46.2%)", size = 5, color = "black",family = "Arial",face = "bold") +
  
  # 添加重叠部分的标签
  annotate("text", x = 3.2, y = 1.5, label = "13 \n (12.3%)", size = 5, color = "black",family = "Arial",face = "bold")+
  theme_void()+
  theme(
...
#
font_add("A","C:/Users/zhimo/AppData/Local/Microsoft/Windows/Fonts/Arial.ttf")   ####字体存放位置
cairo_pdf("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/sensity/sensity_4_mean_line_pfas/VEEN_meeting_mediator.pdf",width = 7, height = 6, pointsize = 20,family = "Arial")
opar <- par(no.readonly = TRUE)
par(pin = c(9,16), mar = c(7,7.5,1,1.5))  ##,mgp=c(3,2,0)
VEEN #出图
