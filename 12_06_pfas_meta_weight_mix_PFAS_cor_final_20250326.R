...
df_background_DNA_LT_meta_mean_final %>% 
select(PFOS:PF3ONS_9CL) %>%
  apply(2, function(x) {     ###对端粒和线粒体拷贝数进行z-score标化
    (x - mean(x)) / sd(x)
  }) %>%
  as_tibble() -> df_background_DNA_LT_meta_mean_final_2


####将z-score标化后的求和，生成混合PFAS暴露的浓度，进行残差分析

df_background_DNA_LT_meta_mean_final_2 %>% 
  rowSums() %>% 
  as_tibble() %>% 
  dplyr::rename("PFAS"="value") %>% 
  cbind(df_background_DNA_LT_meta_mean_final)  %>% 
  select(number:M625,PFAS)-> df_background_DNA_LT_meta_mean_final_3


names(df_background_DNA_LT_meta_mean_final_3)

# 
# 
# df_background_DNA_LT_meta_mean_final <- read_excel("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/metabolomics/pfas_meta_stl_20241216/pfas_meta_result/Mix_PFAS_RAW_DATA.xlsx")
...
###
df_background_DNA_LT_meta_mean_final_3 -> df_background_DNA_LT_meta_mean_final
...
###1.2首先构建sample_info
sample_info <- data.frame(df_background_DNA_LT_meta_mean_final[,c(1:24,296)] %>% 
...
####>>>>>>>>>>>>>>2.对混合PFAS进行spearman回归分析<<<<<<<<<<<<<<<<<<<<<<<
cor_data <-
  lapply(seq_along(rownames(temp_object)), function(i){
    value <- as.numeric(expression_data[i,]) 
    cor_result <- cor.test(value,temp_object@sample_info[,c(25)], method = "spearman")
...
cor_data %>% 
  write.xlsx("result/20240407_PFOA_sperm/Multi_linear_regression_20240407/metabolomics/pfas_meta_stl_20241216/pfas_meta_result/result_mix_PFAS_cor.xlsx")
...
for (idx in 1:271) {
  ...
  permutation_cor_data <-
    ...
    purrr::map(function(i) {
      ...
      cor_result <-
        cor.test(value, sample(temp_object@sample_info$PFAS,
...
linear_model_data <-
  ...
  purrr::map(function(i) {
    ...
    lm_result <-
      glm(formula = PFAS ~ value, data = temp_data)
...
write.xlsx(temp_object@variable_info,"result/20240407_PFOA_sperm/Multi_linear_regression_20240407/metabolomics/pfas_meta_stl_20241216/pfas_meta_result/cor_mix_pfas_meta.xlsx")
...
subject_info <-
  sample_info %>%
  dplyr::select(sample_id,PFAS) %>%
  dplyr::filter(!is.na(PFAS)) %>%
  dplyr::distinct(sample_id, .keep_all = TRUE)

###提取出第一主成分

lapply(1:161, function(i){
  data.frame(
...
df_pca %>% 
  ...
  dplyr::filter(!is.na(PFAS)) -> temp_data 
...
omics_color <-
  c(
    "PFAS" = RColorBrewer::brewer.pal(n = 12, name = "Set3")[4]
  )
...
temp_data %>%
  ...
  annotate("text", 
           x = Inf, y = Inf, label = "Abs(Spearman cor) = 0.12; P Value < 0.01", 
...
cor.test(temp_data$PFAS,temp_data$PC,   method = "spearman")
